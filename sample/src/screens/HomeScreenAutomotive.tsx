...
const HomeScreenAutomotive = ({ navigation }: { navigation: any }) => {
  ...
  useEffect(() => {
    const fetchproduct = () => {
      ...
      const graphql = JSON.stringify({
        query: `query MyQuery {
        collection(id: "gid://shopify/Collection/331367153817") {
        products(first: 10) {
          nodes {
            id
            images(first: 10) {
  ...
  useEffect(() => {
    const fetchproduct = () => {
      ...
      const graphql = JSON.stringify({
        query: `query MyQuery {
        collection(id: "gid://shopify/Collection/331367317657") {
          products(first: 4) {
            nodes {
              id
              images(first: 4) {
...
export default HomeScreenAutomotive;
